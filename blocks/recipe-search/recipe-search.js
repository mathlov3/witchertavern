...
import { getSearchQuery, SEARCH_QUERY_EVENT } from '../../scripts/utils/search.js';
 ...
 */
function createAlgoliaProvider(cfg) {
  async function fetch$(query, facetFilters, fetchFacets = false) {
    ...
    const res = await fetch(
      `https://${cfg.appId}-dsn.algolia.net/1/indexes/${cfg.indexName}/query`,
      {
        method: 'POST',
        headers: {
...
export default async function decorate(block) {
  ...
  if (source === 'query-index') {
    ...
    provider = createQueryIndexProvider(indexUrl);
  } else {
    provider = createAlgoliaProvider({
